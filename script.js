...
function getSmartQuoteFileMessage(ext) { if(ext==='dst')return'Ready-to-stitch embroidery file selected.'; if(ext==='emb')return'Hatch Embroidery project selected.'; if(['pes','exp','jef','vp3','hus','xxx'].includes(ext))return'Embroidery machine file selected for review.'; if(['png','jpg','jpeg','webp','svg'].includes(ext))return'Artwork selected. This design may require digitizing.'; return'Artwork selected for review.'; }
function resetSmartQuoteUploadedFile(){ smartQuoteState.uploadedPath=''; smartQuoteState.uploadedFileName=''; smartQuoteState.uploadedFileSize=0; }
function clearSmartQuoteFile(){ if(smartQuoteState.filePreviewUrl)URL.revokeObjectURL(smartQuoteState.filePreviewUrl); smartQuoteState.file=null; smartQuoteState.filePreviewUrl=''; resetSmartQuoteUploadedFile(); renderUploadStep(); }
function setSmartQuoteFile(file){ smartQuoteError.textContent=''; if(!file)return; const ext=getSmartQuoteFileExtension(file.name); if(!SMART_QUOTE_ACCEPTED_EXTENSIONS.includes(ext)){smartQuoteError.textContent='That file type is not accepted. Please upload artwork or an embroidery file.';return;} if(file.size>SMART_QUOTE_MAX_FILE_SIZE){smartQuoteError.textContent='The file is larger than 10 MB.';return;} if(smartQuoteState.filePreviewUrl)URL.revokeObjectURL(smartQuoteState.filePreviewUrl); smartQuoteState.file=file; smartQuoteState.filePreviewUrl=['png','jpg','jpeg','webp','svg'].includes(ext)?URL.createObjectURL(file):''; resetSmartQuoteUploadedFile(); renderUploadStep(); }
...
function renderReviewStep(){ const rows=[['Product',smartQuoteState.product],['Method',smartQuoteState.method],['Quantity',smartQuoteState.exactQuantity||smartQuoteState.quantityRange],['Placements',smartQuoteState.placements.join(', ')],['Items supplied by',smartQuoteState.garmentSource],['Needed by',smartQuoteState.deadline||'Not specified'],['Name',smartQuoteState.name],['Phone',smartQuoteState.phone],['Email',smartQuoteState.email||'Not provided'],['Preferred contact',smartQuoteState.contactPreference],['Design file',smartQuoteState.file?`${smartQuoteState.file.name} (${formatSmartQuoteFileSize(smartQuoteState.file.size)})`:'Not provided'],['Details',smartQuoteState.details]]; const note=smartQuoteState.file?'Your design will be securely uploaded before WhatsApp opens. Imaginable Things will receive a download link for the original file.':'Confirm the details, then open WhatsApp to send them.'; smartQuoteContent.innerHTML=`<div class="smart-quote-question"><h3>Review your quote request</h3><p>${note}</p></div><div class="smart-quote-review">${rows.map(([l,v])=>`<div class="smart-quote-review-row"><strong>${l}</strong><span>${escapeSmartQuoteText(v)}</span></div>`).join('')}</div>`; }
...
function renderSmartQuoteStep(){ if(!smartQuoteContent)return; const total=9, visible=smartQuoteState.step+1; smartQuoteStepLabel.textContent=`Step ${visible} of ${total}`; smartQuoteProgressBar.style.width=`${visible/total*100}%`; smartQuoteBack.disabled=smartQuoteState.step===0; smartQuoteNext.textContent=smartQuoteState.step===total-1?(smartQuoteState.file?'Upload Design & Open WhatsApp':'Open WhatsApp'):'Next'; smartQuoteError.textContent=''; if(smartQuoteState.step<=2)renderOptionStep(smartQuoteSteps[smartQuoteState.step]); else if(smartQuoteState.step===3)renderPlacementStep(); else if(smartQuoteState.step===4)renderSupplyStep(); else if(smartQuoteState.step===5)renderProjectDetailsStep(); else if(smartQuoteState.step===6)renderUploadStep(); else if(smartQuoteState.step===7)renderContactStep(); else renderReviewStep(); }

async function uploadSmartQuoteFile(){
  const file=smartQuoteState.file;
  if(!file)return null;
  if(smartQuoteState.uploadedPath&&smartQuoteState.uploadedFileName===file.name&&smartQuoteState.uploadedFileSize===file.size){
    return { pathname:smartQuoteState.uploadedPath, downloadUrl:`/api/quote-file?path=${encodeURIComponent(smartQuoteState.uploadedPath)}` };
  }

  const signResponse=await fetch('/api/quote-upload-url',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fileName:file.name,size:file.size,contentType:file.type||''})});
  const signData=await signResponse.json().catch(()=>({}));
  if(!signResponse.ok||!signData.presignedUrl||!signData.pathname){throw new Error(signData.error||'We could not prepare the design upload.');}

  const uploadHeaders=file.type?{'Content-Type':file.type}:{};
...
async function sendSmartQuoteToWhatsApp(){
  ...
  const originalText=smartQuoteNext.textContent;
  smartQuoteNext.disabled=true;
  smartQuoteNext.textContent=smartQuoteState.file?'Uploading design…':'Opening WhatsApp…';
  ...
  try{
    ...
    if(popup){popup.location.href=whatsappUrl;}else{window.location.href=whatsappUrl;}
  }catch(error){
    if(popup&&!popup.closed)popup.close();
    smartQuoteError.textContent=error?.message||'The design could not be uploaded. Please try again.';
  }finally{
    smartQuoteNext.disabled=false;
    smartQuoteNext.textContent=originalText;
  }
}
...
smartQuoteNext?.addEventListener('click',async()=>{saveCurrentSmartQuoteStep();if(!validateSmartQuoteStep())return;if(smartQuoteState.step===8){await sendSmartQuoteToWhatsApp();return;}smartQuoteState.step+=1;renderSmartQuoteStep();});
